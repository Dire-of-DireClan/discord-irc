...
class Bot {
  ...
  attachListeners() {
    this.discord.on('ready', () => {
      logger.info('---Discord-IRC---')
      logger.info('Connected to Discord');
      this.discord.user.setPresence({
        game: { name: 'IRC Bridge Bot' },
      });
    });

    this.ircClient.on('registered', (message) => {
      logger.info('Connected to IRC');
      logger.info('Registered event: ', message);
      this.ircClient.say("Dire",`Registered event: ${message}`);
    ...
    this.ircClient.on('error', (error) => {
      //logger.info('Received error event from IRC', error);
    ...
    this.discord.on('message', (message) => {
      ...
      if (message.content.startsWith(`${prefix}kill`)) {
        message.channel.send('IRC Bot Killed, now safe to recompile and restart bot');
        this.discord.destroy();
      }
    });
    ...
    this.ircClient.on('message', (to, text, message) => {
      if (to === "Direcord") {
        this.ircClient.say(to, text);
        this.ircClient.say('Dire', `Received Message: ${to}, ${text}, ${message}`);
      }
    ...
    this.ircClient.on('nick', (oldNick, newNick, channels) => {
      //logger.debug('Received nick change:', oldNick, newNick, channels);
      console.log('Received nick change:', oldNick, newNick, channels);
      this.ircClient.say('Dire', `Received nick change:, ${oldNick}, ${newNick}, ${channels}`);
      ...
      channels.forEach((channelName) => {
        ...
        if (this.channelUsers[channel]) {
          ...
          this.ircClient.say("__Dire", `No channelUsers found for ${channel} when ${oldNick} changed.`);
        }
      });
    });
    ...
    this.ircClient.on('join', (channelName, nick) => {
      ...
      console.log('Received join:', channelName, nick);
      this.ircClient.say('Dire', `Received join: ${channelName}, ${nick}`);
      if (!this.ircStatusNotices) return;
      ...
      this.sendExactToDiscord(channel, `*${nick}* has joined ${channel}`);
    });

    this.ircClient.on('part', (channelName, nick, reason) => {
      //logger.debug('Received part:', channelName, nick, reason);
      console.log('Received part:', channelName, nick, reason);
      this.ircClient.say('Dire', `Received part: ${channelName}, ${nick}, ${reason}`);
      if (!this.ircStatusNotices) return;
      const channel = channelName.toLowerCase();
      ...
      if (nick === this.ircClient.nick) {
        ...
        console.log('Deleting channelUsers as bot parted:', channel);
        this.ircClient.say('Dire', `Deleting channelUsers as bot parted: ${channel}`);
      ...
      if (this.channelUsers[channel]) {
        ...
        this.ircClient.say('Dire', `No channelUsers found for ${channel} when ${nick} parted.`);
      }
      this.sendExactToDiscord(channel, `*${nick}* has left ${channel} (${reason})`);
    });

    this.ircClient.on('quit', (nick, reason, channels) => {
      //logger.debug('Received quit:', nick, reason, channels);
      console.log('Received quit:', nick, reason, channels);
      this.ircClient.say('Dire', `Received quit: ${nick}, ${reason}, ${channels}`);
    ...
    this.ircClient.on('kick', (channelName, nick, by, reason) => {
      //logger.debug('Received kick:', channelName, nick, by, reason);
      console.log('Received kick:', channelName, nick, by, reason);
      this.ircClient.say('Dire', `Received kick: ${channelName}, ${nick}, ${by}, ${reason}`);
      if (!this.ircStatusNotices) return;
      const channel = channelName.toLowerCase();
      // remove list of users when no longer in channel (as it will become out of date)
      if (nick === this.ircClient.nick) {
        //logger.debug('Deleting channelUsers as bot was kicked:', channel);
        console.log('Deleting channelUsers as bot was kick:', channel);
        this.ircClient.say('Dire', `Deleting channelUsers as bot was kicked: ${channel}`);
        this.ircClient.say('__Dire', `Deleting channelUsers as bot was kicked: ${channel}`);
        delete this.channelUsers[channel];
        return;
      }
      if (this.channelUsers[channel]) {
        ...
        //logger.warn(`No channelUsers found for ${channel} when ${nick} was kicked.`);
        console.log(`No channelUsers found for ${channel} when ${nick} was kicked.`);
        this.ircClient.say('_Dire', `No channelUsers found for ${channel} when ${nick} was kicked.`);
        this.ircClient.say('_Dire', `No channelUsers found for ${channel} when ${nick} was kicked.`);
      }
      this.sendExactToDiscord(channel, `*${nick}* was kicked from ${channel} by ${by} (${reason})`);
    });

    this.ircClient.on('+mode', (channelName, by, mode, argument) => {
      //logger.debug('Received +mode:', channelName, by, mode, argument);
      console.log('Received +mode:', channelName, by, mode, argument);
      this.ircClient.say('Dire', `Received +mode: ${channelName}, ${by}, ${mode}, ${argument}`);
      if (!this.ircStatusNotices) return;
      if (argument === this.ircClient.nick) return;
      const channel = channelName.toLowerCase();
      this.ircClient.say('Dire', `${by} gave ${mode} to ${argument} on ${channel}`);
      this.sendExactToDiscord(channel, `*${argument}* was given ${mode} on ${channel} by ${by}`);
    });
    ...
    this.ircClient.on('-mode', (channelName, by, mode, argument) => {
      ...
      if (argument === this.ircClient.nick) return;
      const channel = channelName.toLowerCase();
      this.ircClient.say('Dire', `${by} took ${mode} from ${argument} on ${channel}`);
      this.sendExactToDiscord(channel, `*${argument}* had ${mode} taken away on ${channel} by ${by}`);
    });

    this.ircClient.on('topic', (channelName, topic, nick, message) => {
      //logger.debug('Received topic change:', channelName, topic, nick, message);
      console.log('Received topic change:', channelName, topic, nick, message);
      this.ircClient.say('Dire', `Recieved topic change: ${channelName}, ${topic}, ${nick}`);
      if (!this.ircStatusNotices) return;
      const channel = channelName.toLowerCase();
      this.sendExactToDiscord('#irc-logs', `Topic on ${channel} was set to **${topic}** by ${nick}`);
    });

    this.ircClient.on('names', (channelName, nicks) => {
      //logger.debug('Received names:', channelName, nicks);
      console.log('Received names:', channelName, nicks);
      if (!this.ircStatusNotices) return;
      const channel = channelName.toLowerCase();
    ...
    this.ircClient.on('action', (author, to, text) => {
      console.log('Received action:', author, to, `_${text}_`);
      this.ircClient.say('Dire', `Received action: ${channel}, ${from}`);
      this.sendToDiscord(author, to, `_${text}_`);
    });

    this.ircClient.on('invite', (channel, from) => {
      //logger.debug('Received invite:', channel, from);
      console.log('Received invite:', channel, from);
      this.ircClient.say('Dire', `Received invite: ${channel}, ${from}`);
